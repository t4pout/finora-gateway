...
import { ShieldCheck, Lock, Star, ChevronDown, ChevronUp, Check } from 'lucide-react';

interface CheckoutV3Props {
  plano: any;
  formData: any;
...
export default function CheckoutV3Component({
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            <div>
              <input type="text" placeholder="Nome completo" value={formData.nome} onChange={(e) => setFormData({ ...formData, nome: e.target.value })}
                style={{ width: '100%', padding: '13px 16px', border: erros.nome ? '2px solid #ef4444' : '2px solid #e5e5e5', borderRadius: '10px', fontSize: '15px', outline: 'none', boxSizing: 'border-box', transition: 'border 0.2s' }}
            ...
            {plano.checkoutPedirEndereco && (
              <div style={{ border: '2px solid #e5e5e5', borderRadius: '10px', overflow: 'hidden' }}>
                <button type="button" onClick={() => setEnderecoAberto(!enderecoAberto)}
                  ...
                  <span>📦 Endereço de entrega {enderecoAberto ? '' : '(clique para preencher)'}</span>
                  {enderecoAberto ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
                </button>
                {enderecoAberto && (
                  <div style={{ padding: '16px', borderTop: '1px solid #e5e5e5', display: 'flex', flexDirection: 'column', gap: '10px' }}>
                    <input type="text" placeholder="CEP" value={formData.cep} onChange={(e) => setFormData({ ...formData, cep: e.target.value })} maxLength={9}
                      style={{ width: '100%', padding: '12px 14px', border: '2px solid #e5e5e5', borderRadius: '8px', fontSize: '14px', outline: 'none', boxSizing: 'border-box' }} />
                    {buscandoCep && <div style={{ fontSize: '12px', color: '#666' }}>Buscando CEP...</div>}
                    <input type="text" placeholder="Rua" value={formData.rua} onChange={(e) => setFormData({ ...formData, rua: e.target.value })}
                      style={{ width: '100%', padding: '12px 14px', border: '2px solid #e5e5e5', borderRadius: '8px', fontSize: '14px', outline: 'none', boxSizing: 'border-box' }} />
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '10px' }}>
                      <input type="text" placeholder="Número" value={formData.numero} onChange={(e) => setFormData({ ...formData, numero: e.target.value })}
                        style={{ padding: '12px 14px', border: '2px solid #e5e5e5', borderRadius: '8px', fontSize: '14px', outline: 'none' }} />
                      <input type="text" placeholder="Complemento" value={formData.complemento} onChange={(e) => setFormData({ ...formData, complemento: e.target.value })}
                        style={{ padding: '12px 14px', border: '2px solid #e5e5e5', borderRadius: '8px', fontSize: '14px', outline: 'none' }} />
                    </div>
                    <input type="text" placeholder="Bairro" value={formData.bairro} onChange={(e) => setFormData({ ...formData, bairro: e.target.value })}
                      style={{ width: '100%', padding: '12px 14px', border: '2px solid #e5e5e5', borderRadius: '8px', fontSize: '14px', outline: 'none', boxSizing: 'border-box' }} />
                    <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '10px' }}>
                      <input type="text" placeholder="Cidade" value={formData.cidade} onChange={(e) => setFormData({ ...formData, cidade: e.target.value })}
                        style={{ padding: '12px 14px', border: '2px solid #e5e5e5', borderRadius: '8px', fontSize: '14px', outline: 'none' }} />
                      <input type="text" placeholder="UF" value={formData.estado} onChange={(e) => setFormData({ ...formData, estado: e.target.value.toUpperCase() })} maxLength={2}
                        style={{ padding: '12px 14px', border: '2px solid #e5e5e5', borderRadius: '8px', fontSize: '14px', outline: 'none' }} />
                    </div>
                  </div>
                )}
