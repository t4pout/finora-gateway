'use client';
// versão 3.0 - multi-layout
export const dynamic = 'force-dynamic';
import { useEffect, useRef, useState } from 'react';
import { useRouter } from 'next/navigation';
import CheckoutV2 from './checkout-v2-component';
...
export default function CheckoutPlanoPage({ params }: { params: Promise<{ linkUnico: string }> }) {
  ...
  useEffect(() => {
    params.then(p => setLinkUnico(p.linkUnico));
  }, [params]);
    ...
    if (typeof window !== 'undefined' && (window as any).fbq) {
      try {
        (window as any).fbq('track', 'InitiateCheckout', {
          content_name: planoData.nome, content_ids: [planoData.id],
          content_type: 'product', value: planoData.preco, currency: 'BRL'
        });
      ...
      if (typeof window !== 'undefined' && (window as any).fbq) {
        try {
          (window as any).fbq('track', 'AddPaymentInfo', {
            content_name: plano.nome, content_ids: [plano.id],
            content_type: 'product', value: plano.preco, currency: 'BRL'
          });
  ...
  useEffect(() => {
    if (!plano?.produto?.id || pixelIniciado.current) return;
    const carregarPixels = async () => {
      try {
        const res = await fetch(`/api/produtos/${plano.produto.id}`);
        if (!res.ok) return;
        const data = await res.json();
        if (data.produto?.pixels?.length > 0) {
          data.produto.pixels.forEach((pixel: any) => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => setTempoRestante(prev => prev <= 1 ? 0 : prev - 1), 1000);
    return () => clearInterval(interval);
  }, [plano, tempoRestante]);

  useEffect(() => {
    if (!plano?.checkoutProvaSocial || !plano.checkoutIntervaloPop) return;
    const masc = ['João', 'Pedro', 'Carlos', 'Rafael', 'Lucas', 'Felipe', 'Bruno', 'Marcelo'];
    const fem = ['Maria', 'Ana', 'Julia', 'Beatriz', 'Camila', 'Larissa', 'Fernanda', 'Paula'];
    const cidades = ['São Paulo', 'Rio de Janeiro', 'Belo Horizonte', 'Brasília', 'Curitiba', 'Porto Alegre'];
    const mostrarPopup = () => {
      let nome = '';
      if (plano.checkoutProvaSocialGenero === 'HOMENS') nome = masc[Math.floor(Math.random() * masc.length)];
      else if (plano.checkoutProvaSocialGenero === 'MULHERES') nome = fem[Math.floor(Math.random() * fem.length)];
      else { const t = [...masc, ...fem]; nome = t[Math.floor(Math.random() * t.length)]; }
      const cidade = cidades[Math.floor(Math.random() * cidades.length)];
      const min = Math.floor(Math.random() * 30) + 1;
      const popup = document.createElement('div');
      popup.className = 'popup-prova-social';
      popup.innerHTML = `<div class="popup-conteudo"><div class="popup-icone">✓</div><div class="popup-texto"><p class="popup-nome">${nome} de ${cidade}</p><p class="popup-acao">Acabou de comprar</p><p class="popup-tempo">há ${min} minutos</p></div></div>`;
    ...
    try {
      const res = await fetch('/api/pagamento', {
        ...
        body: JSON.stringify({
          ...
          compradorCpf: formData.cpf, compradorTel: formData.telefone,
  ...
  if (!plano) return <div className="loading-container"><p className="error-text">Plano não encontrado</p></div>;

  // =============================================
  // ROTEAMENTO DE VERSÃO
  // =============================================
  const versao = plano.checkoutVersao || 'v1';
      ...
      <div className="checkout-container" style={{ background: `linear-gradient(135deg, ${corSecundaria} 0%, ${corPrimaria} 100%)` }}>
        <div className="checkout-wrapper">
          ...
          {plano.checkoutLogoSuperior && (
            <div className="logo-superior"><img src={plano.checkoutLogoSuperior} alt="Logo" /></div>
          )}
                  ...
                  <div className="form-row">
                    ...
                    <div className="form-group">
                      ...
                      <input
                        ...
                        placeholder="000.000.000-00" className="form-input" maxLength={14}
                      />
                    </div>
                  </div>
                  <button onClick={avancarEtapa1} className="btn-primary" style={{ backgroundColor: corPrimaria }}>Continuar →</button>
                </div>
              )}
          ...
          {plano.checkoutLogoInferior && (
            <div className="logo-inferior"><img src={plano.checkoutLogoInferior} alt="Logo" /></div>
          )}
        </div>
      </div>
      <style jsx global>{`
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .cronometro-mensagem { font-size: 14px; font-weight: 500; margin-bottom: 8px; }
        ...
        .progress-bar { display: flex; align-items: center; justify-content: center; padding: 32px; }
        ...
        .produto-info { display: flex; gap: 20px; padding: 24px; background: #f9fafb; border-radius: 16px; align-items: center; }
        ...
        .btn-secondary { padding: 16px; border: 2px solid #e5e7eb; border-radius: 12px; font-size: 16px; font-weight: 600; color: #6b7280; background: white; cursor: pointer; }
