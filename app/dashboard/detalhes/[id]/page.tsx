...
const carregarCoProdutores = async () => {
  ...
  try {
    const res = await fetch(`/api/produto/${produtoId}/fretes`);
  ...
  <div className="max-w-4xl mx-auto">
    <div className="bg-white dark:bg-finoradark-card rounded-xl border border-gray-200 dark:border-finoradark-border p-8">
      ...
      {fretes.length === 0 ? (
        <div className="text-center py-16 bg-gray-50 dark:bg-finoradark-card2 rounded-xl">
          <div className="text-6xl mb-4">📦</div>
          <h3 className="text-xl font-bold text-gray-900 dark:text-finoradark-text mb-2">Nenhuma opção de frete cadastrada</h3>
          <p className="text-gray-600 dark:text-finoradark-textmuted mb-6">Ex: Frete Grátis (10 dias), SEDEX (3 dias)</p>
          <button onClick={() => setModalFrete({ aberto: true, frete: null })} className="px-6 py-3 bg-purple-600 dark:bg-finoradark-glow text-white rounded-lg font-semibold hover:bg-purple-700 dark:hover:opacity-90 transition">Criar Primeira Opção</button>
        </div>
      ) : (
        <div className="space-y-4">
          {fretes.map((f) => (
            <div key={f.id} className="border-2 border-gray-200 dark:border-finoradark-border rounded-xl p-6 hover:border-purple-400 transition">
              <div className="flex items-center justify-between">
                <div className="flex-1">
                  <h3 className="text-lg font-bold text-gray-900 dark:text-finoradark-text">{f.nome} {!f.ativo && <span className="text-sm font-normal text-gray-400">(inativo)</span>}</h3>
                  {f.descricao && <p className="text-gray-600 dark:text-finoradark-textmuted text-sm mt-1">{f.descricao}</p>}
                  <div className="text-sm text-gray-500 dark:text-finoradark-textmuted mt-1">Prazo: {f.prazoDias} dia(s)</div>
    ...
    {modalFrete.aberto && (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4" onClick={() => setModalFrete({ aberto: false, frete: null })}>
        <div className="bg-white dark:bg-finoradark-card rounded-2xl p-8 max-w-md w-full" onClick={(e) => e.stopPropagation()}>
          <h3 className="text-2xl font-bold text-gray-900 dark:text-finoradark-text mb-6">{modalFrete.frete ? 'Editar Opção de Frete' : 'Nova Opção de Frete'}</h3>
          <div className="space-y-4">
            <div><label className="block text-sm font-semibold text-gray-900 dark:text-finoradark-text mb-2">Nome *</label><input type="text" value={formFrete.nome} onChange={(e) => setFormFrete({...formFrete, nome: e.target.value})} className="w-full px-4 py-3 border border-gray-300 dark:border-finoradark-border dark:bg-finoradark-card2 dark:text-finoradark-text rounded-lg focus:ring-2 focus:ring-purple-600 outline-none" placeholder="Ex: Frete Grátis" /></div>
            <div><label className="block text-sm font-semibold text-gray-900 dark:text-finoradark-text mb-2">Descrição</label><input type="text" value={formFrete.descricao} onChange={(e) => setFormFrete({...formFrete, descricao: e.target.value})} className="w-full px-4 py-3 border border-gray-300 dark:border-finoradark-border dark:bg-finoradark-card2 dark:text-finoradark-text rounded-lg focus:ring-2 focus:ring-purple-600 outline-none" placeholder="Ex: Entrega garantida" /></div>
            ...
            <div className="flex gap-3 pt-4">
              <button onClick={() => setModalFrete({ aberto: false, frete: null })} className="flex-1 px-6 py-3 border-2 border-gray-300 dark:border-finoradark-border text-gray-700 dark:text-finoradark-textmuted rounded-lg font-semibold">Cancelar</button>
              <button onClick={async () => {
                if (!formFrete.nome || !formFrete.prazoDias) { alert('Preencha nome e prazo'); return; }
                const url = modalFrete.frete ? `/api/produto/${produtoId}/fretes/${modalFrete.frete.id}` : `/api/produto/${produtoId}/fretes`;
                const method = modalFrete.frete ? 'PUT' : 'POST';
                await fetch(url, { method, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(formFrete) });
                alert(modalFrete.frete ? 'Atualizado!' : 'Criado!');
                setModalFrete({ aberto: false, frete: null });
                carregarFretes();
              }} className="flex-1 px-6 py-3 bg-purple-600 dark:bg-finoradark-glow text-white rounded-lg font-semibold hover:bg-purple-700 dark:hover:opacity-90 transition">{modalFrete.frete ? 'Salvar' : 'Criar'}</button>
            </div>
          </div>
        </div>
      </div>
